...
import { EntityDefinitionSchema, type EntityDefinitionJson } from '../schemas'
...
class EntityManager extends FileLoader<EntityDefinition, EntityDefinitionJson> {
  ...
  protected prepareForSave(data: EntityDefinition): EntityDefinitionJson {
    ...
    const { filePath, ...jsonData } = data as any
    return jsonData
  }
   ...
   */
  protected async postProcess(validated: EntityDefinitionJson, filePath: string): Promise<EntityDefinition> {
  ...
  applyTransformRecursive(
    ...
    parentTransform: Transform = { x: 0, y: 0, rotation: 0, scaleX: 1, scaleY: 1 }
  ): Transform {
