...
export class ReferenceManager {
	...
	async findReferences(
		filePath: string,
		projectDir: string,
	): Promise<Map<string, ReferenceType[]>> {
		...
		if (isDirectory) {
			...
			for (const [refFilePath, refTypes] of references.entries()) {
				try {
					...
					console.error(
						`Failed to update references in ${refFilePath}:`,
						error,
					);
				}
			}
	 ...
	 */
	private updateManagerCachesForDirectory(
		oldDirPath: string,
		newDirPath: string,
	): void {
		...
		try {
			...
			if (projectData.tilesets) {
				for (const tilesetPath of projectData.tilesets) {
					...
					if (
						fileManager.normalize(absoluteTilesetPath) ===
						fileManager.normalize(targetPath)
					) {
			...
			if (projectData.maps) {
				for (const mapPath of projectData.maps) {
					...
					if (
						fileManager.normalize(absoluteMapPath) ===
						fileManager.normalize(targetPath)
					) {
	...
	private async scanTilesetFiles(
		...
		for (const tilesetFile of tilesetFiles) {
			try {
				...
				if (tilesetData.imagePath) {
					// Resolve image paths relative to project directory (all paths are relative to assets root)
					const absoluteImagePath = fileManager.isAbsolute(
						tilesetData.imagePath,
					)
						? tilesetData.imagePath
						: fileManager.join(projectDir, tilesetData.imagePath);
					...
					if (
						fileManager.normalize(absoluteImagePath) ===
						fileManager.normalize(targetPath)
					) {
						const refTypes: ReferenceType[] = [
							{
