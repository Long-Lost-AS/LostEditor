...
export const ResourceBrowser = ({ onClose }: ResourceBrowserProps) => {
	...
	// Native drag-and-drop state (for external files from Finder/Explorer)
	...
	const confirmRename = async () => {
		...
		try {
			...
			if (fileExists) {
				setError(
					`A ${item.isDirectory ? "folder" : "file"} with that name already exists`,
				);
			...
			if (projectDirectory && item.path.startsWith(projectDirectory)) {
				try {
					await referenceManager.updateReferences(
						item.path,
						newPath,
						projectDirectory,
					);
					...
					try {
						...
						if (fileExists) {
							const confirmed = confirm(
								`File "${fileName}" already exists in destination. Replace it?`,
							);
	...
	const UpDirectoryDropTarget = () => {
		...
		return (
			...
			>
				...
				<div
					className="text-xs text-center break-all line-clamp-2 w-full"
					style={{ color: "#cccccc", userSelect: "none" }}
				>
					..
				</div>
			</div>
		...
		return (
			...
			>
				...
				{isSelected && selectedItems.size > 1 && (
					...
					>
						{Array.from(selectedItems).indexOf(item.path) === 0
							? selectedItems.size
							: ""}
					</div>
				)}
		...
		>
			<div className="h-full flex flex-col" style={{ background: "#252526" }}>
				...
				<div
					...
					style={{ background: "#2d2d30", borderBottom: "1px solid #3e3e42" }}
				>
