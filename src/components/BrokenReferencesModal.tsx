...
export const BrokenReferencesModal = ({
	references,
	projectDir,
	onClose,
	onContinue,
	...
	const handleFix = async (ref: BrokenRefWithStatus, index: number) => {
		...
		setRefs((prev) =>
			prev.map((r, i) =>
				i === index ? { ...r, status: "fixing" as const } : r,
			),
		);
		...
		try {
			...
			if (result.canceled || !result.filePaths?.[0]) {
				...
				setRefs((prev) =>
					prev.map((r, i) =>
						i === index ? { ...r, status: "pending" as const } : r,
					),
				);
				return;
			}
			...
			const affectedFilesMap = await referenceManager.findReferences(
				ref.expectedPath,
				projectDir,
			);
			...
			>
				...
				<div className="mb-4">
					...
					<p className="text-gray-400 text-sm">
						{fixedCount} of {totalCount} fixed • Click on any item to locate the
						missing file
					</p>
				</div>
					...
					<button
						...
						title={
							allFixed
								...
								: "Fix all references before continuing"
						}
					>
						Continue {allFixed && "✓"}
					</button>
