...
export const EntitySelectMenu = ({ isOpen, onClose }: EntitySelectMenuProps) => {
  const { setSelectedEntityDefId, setSelectedTilesetId, getTilesetById, getActiveMapTab, updateTabData } = useEditor()
  ...
  const handleSelectEntity = useCallback((entity: EntityDefinition) => {
  ...
  }, [setSelectedEntityDefId, setSelectedTilesetId, getActiveMapTab, updateTabData, onClose])
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20">
      ...
      >
              ...
              <>
                ...
                <div className="p-4 border-t border-gray-700">
                  <div className="text-sm font-semibold mb-2" style={{ color: '#cccccc' }}>
                    {filteredEntities[selectedIndex].name || filteredEntities[selectedIndex].id}
                  </div>
                  <div className="text-xs space-y-1" style={{ color: '#858585' }}>
                    {filteredEntities[selectedIndex].sprites && filteredEntities[selectedIndex].sprites!.length > 0 && (
                      <div>Sprites: {filteredEntities[selectedIndex].sprites!.length}</div>
                    )}
                    {filteredEntities[selectedIndex].colliders && filteredEntities[selectedIndex].colliders!.length > 0 && (
                      <div className="text-green-400">
                        Colliders: {filteredEntities[selectedIndex].colliders!.length}
                      </div>
                    )}
                    {filteredEntities[selectedIndex].properties && Object.keys(filteredEntities[selectedIndex].properties!).length > 0 && (
                      <div>Properties: {Object.keys(filteredEntities[selectedIndex].properties!).length}</div>
                    )}
                  </div>
                </div>
              </>
        ...
        >
          <div>
            {filteredEntities.length > 0 && (
              <span>{filteredEntities.length} entit{filteredEntities.length !== 1 ? 'ies' : 'y'}</span>
            )}
          </div>
