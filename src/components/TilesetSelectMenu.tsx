...
export const TilesetSelectMenu = ({ isOpen, onClose }: TilesetSelectMenuProps) => {
  const { setCurrentTileset, setSelectedTilesetId, setSelectedTileId, setSelectedTile, getActiveMapTab, updateTabData } = useEditor()
  ...
  const handleSelectTileset = useCallback((tileset: Tileset) => {
  ...
  }, [setCurrentTileset, setSelectedTilesetId, setSelectedTileId, setSelectedTile, getActiveMapTab, updateTabData, onClose])
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      switch (e.key) {
        ...
        case 'ArrowDown':
          e.preventDefault()
          setSelectedIndex((prev) => Math.min(prev + 1, filteredTilesets.length - 1))
          break
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20">
      ...
      <div
        ...
        style={{ background: '#2d2d30', border: '1px solid #3e3e42', height: '500px' }}
      >
        {/* Search Input */}
        ...
        <div className="flex flex-1 min-h-0">
          ...
          >
            {filteredTilesets.length > 0 && filteredTilesets[selectedIndex] ? (
              <>
                ...
                <div className="p-4 border-t border-gray-700">
                  <div className="text-sm font-semibold mb-2" style={{ color: '#cccccc' }}>
                    {filteredTilesets[selectedIndex].name || filteredTilesets[selectedIndex].id}
                  </div>
                  <div className="text-xs space-y-1" style={{ color: '#858585' }}>
                    <div>
                      Tile Size: {filteredTilesets[selectedIndex].tileWidth}×{filteredTilesets[selectedIndex].tileHeight}px
                    </div>
                    {filteredTilesets[selectedIndex].imageData && (
                      <>
                        <div>
                          Image: {filteredTilesets[selectedIndex].imageData!.width}×{filteredTilesets[selectedIndex].imageData!.height}px
                        </div>
                        <div>
                          Grid: {Math.floor(filteredTilesets[selectedIndex].imageData!.width / filteredTilesets[selectedIndex].tileWidth)}×{Math.floor(filteredTilesets[selectedIndex].imageData!.height / filteredTilesets[selectedIndex].tileHeight)} tiles
                        </div>
                        <div>
                          Total Tiles: {Math.floor(filteredTilesets[selectedIndex].imageData!.width / filteredTilesets[selectedIndex].tileWidth) * Math.floor(filteredTilesets[selectedIndex].imageData!.height / filteredTilesets[selectedIndex].tileHeight)}
                        </div>
                      </>
                    )}
                    {filteredTilesets[selectedIndex].terrainLayers && filteredTilesets[selectedIndex].terrainLayers!.length > 0 && (
                      <div className="text-green-400">
                        Terrain Layers: {filteredTilesets[selectedIndex].terrainLayers!.length}
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="flex items-center justify-center h-full text-sm" style={{ color: '#858585' }}>
                No tileset selected
              </div>
            )}
          </div>
        ...
        >
          <div>
            {filteredTilesets.length > 0 && (
              <span>{filteredTilesets.length} tileset{filteredTilesets.length !== 1 ? 's' : ''}</span>
            )}
          </div>
