...
export const TilesetCreator = ({
		...
		if (mode === "tiles") {
			...
			tiles.forEach((tile) => {
				...
				ctx.strokeRect(
					tile.x * scale,
					tile.y * scale,
					(tile.width || tileWidth) * scale,
					(tile.height || tileHeight) * scale,
				);
			});
		} else {
			...
			entities.forEach((entity) => {
				...
				ctx.lineWidth = 2;
				ctx.strokeRect(
					entity.sprite.x * scale,
					entity.sprite.y * scale,
	...
	const getCanvasCoords = (e: React.MouseEvent<HTMLCanvasElement>) => {
		...
		const x = Math.floor(
			((e.clientX - rect.left) / rect.width) * (sourceImage?.width || 0),
		);
		const y = Math.floor(
			((e.clientY - rect.top) / rect.height) * (sourceImage?.height || 0),
		);
		return { x, y };
	};
	...
	const handleCollisionUpdate = (collision: {
		...
		if (mode === "tiles") {
			setTiles(
				tiles.map((t) =>
					t.tempId === selectedItemId ? { ...t, collision } : t,
				),
			);
		} else {
			setEntities(
				entities.map((e) =>
					e.tempId === selectedItemId ? { ...e, collision } : e,
				),
			);
		}
	};
	...
	const handleSave = async () => {
		...
		const finalTiles: TileDefinition[] = tiles.map(
			({ tempId, ...tile }) => tile,
		);
		const finalEntities: EntityDefinition[] = entities.map(
			({ tempId, ...entity }) => entity,
		);
			...
			{isEditingCollision && selectedItem && sourceImage && (
				<div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
					<div className="bg-gray-800 p-4 rounded-lg">
						<CollisionEditor
							width={
								"sprite" in selectedItem
									? selectedItem.sprite.width
									: selectedItem.width || tileWidth
							}
							height={
								"sprite" in selectedItem
									? selectedItem.sprite.height
									: selectedItem.height || tileHeight
							}
							...
							backgroundRect={{
								x:
									"sprite" in selectedItem
										? selectedItem.sprite.x
										: selectedItem.x,
								y:
									"sprite" in selectedItem
										? selectedItem.sprite.y
										: selectedItem.y,
								width:
									"sprite" in selectedItem
										? selectedItem.sprite.width
